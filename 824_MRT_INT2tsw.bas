...
#define t1_824_Blinky_Dwell		200
#define t2_824_Blinky_Dwell		250
#define t3_824_Blinky_Dwell		300
'~
...
sub Init_MRT (MRT_Counts as integer, MRT_ISR_Addr as integer)
...
sub task0			' task0 is the first called, using main stack, and handles an exit semi-gracefully
	...
	while 1					' tasks SHALL NEVER return -- equivalent of crossing the streams
		x += 1
		print 0
		if x > &H1000 then exit	' after ~1M iterations through this loop, exit the loop - special case for task 0 (uses MSP)
		wait(100)
	loop							' this takes ~10 seconds on a 100MHz 54102.
	
	VICIntEnClear = (1<<10)				' disable the MRT interrupt
	Print "Task 0 initiating a termination of the app @ ";timer
...
sub task1
	...
	do
		i += 1
		out(t1_824_Blinky_led_port) = I and 1
		print 1;
		wait(t1_824_Blinky_Dwell)	
	loop
	endsub
...
sub task2
	...
	do
		i += 1
		out(t2_824_Blinky_led_port) = I and 1
		print 2;
		wait(t2_824_Blinky_Dwell)	
	loop
	endsub
...
sub task3
	...
	do
		i += 1
		out(t3_824_Blinky_led_port) = I and 1
		print 3;
		wait(t3_824_Blinky_Dwell)	
	loop
	endsub
...
sub Init_Tasks
	...
	taskTIME(0) = 10		' arbitrary time in mS for each task
	taskTIME(1) = 10
	taskTIME(2) = 10
	taskTIME(3) = 10
