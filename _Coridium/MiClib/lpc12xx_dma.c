...
{
	LPC_SYSCON->PRESETCTRL &= ~(1 << 10); //reset DMA
	LPC_SYSCON->PRESETCTRL |= (1 << 10);//de assert DMA reset
	LPC_SYSCON->SYSAHBCLKCTRL |= (1 << 17);//enable DMA
...
void DMA_ControlDataConfig(DMA_ControlDataConfigTypeDef *ConfigData)
{
	...
	if(ConfigData->PriAltStructure)
	{
		...
		{	
			if (ConfigData->SrcDataIncSize != DMA_DATA_INC_NO) {
			*(uint32_t *)
				(LPC_DMA->ALT_CTRL_BASE_PTR + (ConfigData->ChannelNum << 4)) =	
					ConfigData->SrcAddr + ((0x01 << ConfigData->SrcDataWidth) * ConfigData->TransferNum)
					 - (0x01 << ConfigData->SrcDataWidth);
			...
			*(uint32_t *)
				(LPC_DMA->ALT_CTRL_BASE_PTR + (ConfigData->ChannelNum << 4)) = ConfigData->SrcAddr;
			}
			if (ConfigData->DstDataIncSize != DMA_DATA_INC_NO) {
			*(uint32_t *)
				(LPC_DMA->ALT_CTRL_BASE_PTR + (ConfigData->ChannelNum << 4) + 4) =
					ConfigData->DstAddr + ((0x01 << ConfigData->DstDataWidth) * ConfigData->TransferNum)
					 - (0x01 << ConfigData->DstDataWidth);
			...
			*(uint32_t *)
				(LPC_DMA->ALT_CTRL_BASE_PTR + (ConfigData->ChannelNum << 4) + 4) = ConfigData->DstAddr;
			}
		}
		*(uint32_t *)
	...
	else
	{
		...
		else
		{
			...
			*(uint32_t *)
				(LPC_DMA->CTRL_BASE_PTR + (ConfigData->ChannelNum << 4)) = ConfigData->SrcAddr;	
			}
			if (ConfigData->DstDataIncSize != DMA_DATA_INC_NO) {
			*(uint32_t *)
				(LPC_DMA->CTRL_BASE_PTR + (ConfigData->ChannelNum << 4) + 4) =
			...
			*(uint32_t *)
				(LPC_DMA->CTRL_BASE_PTR + (ConfigData->ChannelNum << 4) + 4) = ConfigData->DstAddr;	
			}
		}
		*(uint32_t *)
