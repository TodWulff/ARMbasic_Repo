...
//	LPC_SYSCON->PDRUNCFG &= 0xBF;//~(0x1<<6);
...
//	LPC_WDT->TC = 0x003FFF;     //about 8 seconds, once WDEN is set, the WDT will start after feeding
//	LPC_WDT->MOD = WDEN | WDRESET;
//	LPC_WDT->FEED = 0xAA;		// Feeding sequence
//	LPC_WDT->FEED = 0x55;    
//	// Check reset status for watchdog reset
