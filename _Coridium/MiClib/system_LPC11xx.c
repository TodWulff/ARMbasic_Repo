...
//       <o8.0..4>   M: PLL Multiplier Selection  // F_pll in range of 156-320 MHz
//                     <1-32><#-1>
...
#ifdef LPC1168
 #define SYSPLLCLKSEL_Val      0x00000001		// use the crystal osc for now
#else 
 #define SYSPLLCLKSEL_Val      0x00000000
#endif
#define SYSPLL_SETUP          1
  // P divider affects Fcco but not the CLKout  -- 43 runs PLL too fast
//#define SYSPLLCTRL_Val        0x00000043	// M low 5 bits 0-0x1F +1, P next 2 0x60 00=2 20=4 40=8 60=16
#define SYSPLLCTRL_Val        0x00000023	// M low 5 bits 0-0x1F +1, P next 2 0x60 00=2 20=4 40=8 60=16
#define MAINCLKSEL_Val        0x00000003
#define SYSAHBCLKDIV_Val      0x00000001	// divide main clock by 2 (50 MHz)
#define AHBCLKCTRL_Val        0x000107DF	// turn on timers, UART gets turned on in UART_init 0x0001005F
#define SSP0CLKDIV_Val        0x00000008
#define UARTCLKDIV_Val        0x00000008
#define SSP1CLKDIV_Val        0x00000008
...
#define __DIVSEL   (((WDTOSCCTRL_Val & 0x1F) << 1) + 2)
    ...
    /* sys_pllclkin calculation */
      ...
      #define  __SYS_PLLCLKOUT         (__SYS_PLLCLKIN * ((SYSPLLCTRL_Val & 0x01F) + 1)) // CLKOUT shows 100 MHz
//#define DEBUG_CLKOUT
 ...
 *----------------------------------------------------------------------------*/
int SystemCoreClock = __IRC_OSC_CLK;/*!< System Clock Frequency (Core Clock)*/
...
void * MEMCPY(void *dst0, void *src0, int len0);  // len0 in bytes
...
void SystemInit (void)
{
  	...
  	#ifdef LPC1168
  			LPC_SYSCON->SYSAHBCLKCTRL |= (1<<16);	// to use IOCON need to turn it on
  	
		#warning *********** WARNING HACK FOR LPC1168 *******************************  
		// use external clock for now 
			* (int *) 0x400440F0 = 1;	// xtal pin
			* (int *) 0x400440F4 = 1;	// xtal pin
...
#ifdef DEBUG_CLKOUT	// drive the CLKOUT pin
  LPC_SYSCON->CLKOUTCLKSEL = 0x3;		// main clock
  LPC_SYSCON->CLKOUTDIV = 100; 			// enable it out and divide by 100
  LPC_SYSCON->CLKOUTUEN = 0; 			// toggle it to
  LPC_SYSCON->CLKOUTUEN = 1;			// update the clkout
  ...
  //Change default pin function to IO
#ifndef TESTING			// let the SW debugger work  
  LPC_IOCON->SWCLK_PIO0_10 = 0xd1;	// IO with pullup   was -- 17;//NOTE: these pins use 1 as IO function
  LPC_IOCON->SWDIO_PIO1_3 = 0xd1;	//	17;
#endif  
  
  LPC_IOCON->R_PIO0_11 = 0xd1;	// IO with pullup  was  -- 17;
  LPC_IOCON->R_PIO1_0 = 0xd1;	// IO with pullup  was  -- 17;
  LPC_IOCON->R_PIO1_1 = 0xd1;	//	17;
  LPC_IOCON->R_PIO1_2 = 0xd1;	//	17;
//  LPC_IOCON->PIO1_4   = 0xd0;		// reset sets this

#ifdef FOR_BASIC
		//copy vector table to RAM
	MEMCPY((void *)0x10000000, (void *)0x00000000, 48<<2);

// can do this manually but not inline for some reason	
	LPC_SYSCON->SYSMEMREMAP = 1; //assign new vector table in RAM, so interrupts can be patched by BASIC
#endif
...
void IO2AD(int chan) {
  #define	IOCON_BASE	0x40044000
	if (chan<4)	// set these pins to analog mode
		*(volatile int *)(IOCON_BASE+0x74+((chan)<<2)) = 0x42;
	else if (chan==4)
		*(volatile int *)(IOCON_BASE+0x90) = 0x42;	// io = 0x90;
	else if (chan==5)
		*(volatile int *)(IOCON_BASE+0x94) = 0x41;	// io = 0x90;
}
