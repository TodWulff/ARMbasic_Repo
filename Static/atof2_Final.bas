'==============================================================================
Function atof(byref strg as string) as single

	// a function for ARMbasic v8.x+ where a string representation of a nhttps://i.imgur.com/O11drii.pngumber is processed and a single is returned.
	...
	dim i, i1, i2, i3, sign, exp_sign, tempint as integer
	...
	for i = 0 to len(strg)-1								// and here we go - clackity clack, then the strange silence as gravity pulls you over the top and down the rails...
		...
		if (strg(i)>="0") and (strg(i)<="9")    		' this used to be oneline, but compiler changes predicate no implicit single type conversion ops on 'derived integers' from strings
			tempint = strg(i)-"0"							' had to add in the use of an integer to make it play w/ v8
		...
		if strg(i) = "."									// ok, we're goin fractional
			...
			for i1 = i to len(strg)-1						// and jump into a sub loop to parse the rest of the string
				...
				if (strg(i1)>="0") and (strg(i1)<="9")		// if the char is part of a number
					...
					tempint = strg(i1)-"0"					' had to add in the use of an integer to make it play w/ v8
				...
				if ((strg(i1)="e") or (strg(i1)="E")) 		// if we arrive at an e or an E then we jump yet again into a sub loop to pocoess e-notation
					...
					i1+=1									// ...
					for i2 = i1 to len(strg)-1				// and grind thru the rest of the string
						...
						if (strg(i2)>="0") and (strg(i2)<="9") 	' this used to be oneline, but compiler changes predicate no implicit single type conversion ops on 'derived integers' from strings
							tempint = strg(i2)-"0"				' had to add in the use of an integer to make it play w/ v8
						...
						if strg(i2) = "."						// yes, I said mantissa as it is possible that someone could get the bright idea to try to do fractional powers of 10...
							...
							i2+=1								// ...
							for i3 = i2 to len(strg)-1			// and here we go with fractional enotation component parsing
								...
								if (strg(i3)>="0") and (strg(i3)<="9")	// if the char is part of a number
									...
									tempint = strg(i3)-"0"				' had to add in the use of an integer to make it play w/ v8
...
main:
	dim inputstring(64) as string
	while 1
		print "Enter a real number (+/- integer or single (with decimal point and right digits (can include +/- e-notation too)) (0.0 to end): ";
		debugin inputstring
		print inputstring
		if inputstring="0.0" then exit
		print 
		print "Value derived: ";atof(inputstring)
		print
		print
