...
SUB ON_TIMER ( max_cnt, dothis )
    ...
    MRT_Channel0_CTRL = 3    		    	' enable TIMERn Interrupt in one shot mode
    MRT_ISR = dothis or 1          			' assign the isr address to the NVIC vector table for MRT_irq  (the 'or 1' is for thumb code purposes)
...
#ifdef LPC824
	MRT_Channel0_CTRL = 2	' disable interrupt	-- leave in one-shot mode
#else	
